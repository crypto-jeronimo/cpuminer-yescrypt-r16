...
static enum algos opt_algo = ALGO_YESCRYPTR16;
...
Options:\n\
  -a, --algo=ALGO       specify the algorithm to use\n\
                          yescryptr16 yescrypt R16 (default)\n\
                          yescrypt yescrypt\n\
...
static void stratum_gen_work(struct stratum_ctx *sctx, struct work *work)
{
	...
	else if (opt_algo == ALGO_YESCRYPTR16)
		diff_to_target(work->target, sctx->job.diff / 65536.0);
	else
	...
	while (1) {
		...
		case ALGO_YESCRYPT:
			rc = scanhash_yescrypt(thr_id, work.data, work.target,
									max_nonce, &hashes_done, false);
			break;

		case ALGO_YESCRYPTR16:
			rc = scanhash_yescrypt(thr_id, work.data, work.target,
									max_nonce, &hashes_done, true);
			break;
...
{
   printf("\n         **********  "PACKAGE_NAME"  *********** \n");
   printf("     A Yescrypt CPU miner supporting R16.\n\n");
   printf("     If you have found this software more useful than other alternatives \n");
   printf("        available, please, feel free to donate!\n");
   printf("     After all, those morning cups of coffee need to come from somewhere,\n");
   printf("        in order to enable writing efficient code :)\n\n");
